...
def pixel2cam(depth, pixel_coords, intrinsics, is_homogeneous=True):   ### 将像素坐标转换为相机坐标系中的（3-D）坐标3/8
    ...
    """
    #print('------------------------------------')   ### add 3/13
    #print(intrinsics)                     ### add 3/13
    #print('------------------------------------')   ### add 3/13
    batch, height, width = depth.get_shape().as_list()
    depth = tf.reshape(depth, [batch, 1, -1])
    pixel_coords = tf.reshape(pixel_coords, [batch, 3, -1])  ### pixel_coords [batch,3,height*width] 3/13
    ...
    #print(pixel_coords)                   ### add 3/13
    #print('------------------------------------')   ### add 3/13
    pixel_coords = tf.reshape(tf.transpose(pixel_coords, [1,0,2]), [3,-1])     ### add 3/13
    cam_coords = tf.matmul(tf.matrix_inverse(intrinsics), pixel_coords)       ### add 3/13
    cam_coords = tf.transpose(tf.reshape(cam_coords, [3,batch,-1]), [1,0,2]) * depth   ### add 3/13
...
def projective_inverse_warp(img, depth, pose, intrinsics):
    ...
    #intrinsics = tf.tile(tf.expand_dims(intrinsics,0),multiples=[batch,1,1])  ### add复制batch次 3/10 删除3/13
    # Convert pose vector to matrix
    pose = pose_vec2mat(pose)      ### shape [batch,4,4] 3/13
    ...
    filler = tf.constant([0.0, 0.0, 0.0, 1.0], shape=[1, 4])    ### [1,1,4]改为[1,4]
    #filler = tf.tile(filler, [batch, 1, 1])         ### 删除3/13
    #intrinsics = tf.concat([intrinsics, tf.zeros([batch, 3, 1])], axis=2) ### 改为下面一句 3/13
    intrinsics = tf.concat([intrinsics, tf.zeros([3, 1])], axis=1)  ### axis由2改为1 3/13
    intrinsics = tf.concat([intrinsics, filler], axis=0)    ### axis由1改为0 3/13
    # Get a 4x4 transformation matrix from 'target' camera frame to 'source'
    # pixel frame.
    pose = tf.reshape(tf.transpose(pose, [1,0,2]), [4,-1])    ### add 3/13
    proj_tgt_cam_to_src_pixel = tf.matmul(intrinsics, pose)   ### K * T 3/8
    proj_tgt_cam_to_src_pixel = tf.transpose(tf.reshape(proj_tgt_cam_to_src_pixel, [4,batch,-1]), [1,0,2]) ### add 3/13
